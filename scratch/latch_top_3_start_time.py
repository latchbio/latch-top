...
argp.add_argument(
    ...
    default=None,
    type=float,
    help="Use base 2 units instead of SI units",
)
argp.add_argument(
...
cgroup_mem_limit_re = re.compile(r"hierarchical_memsw_limit ([0-9]+)", re.IGNORECASE)
memory_stats = Path("/sys/fs/cgroup/memory/memory.stat").read_text()
memory_limit = int(cgroup_mem_limit_re.search(memory_stats).group(1))
...
def run():
    ...
    def take_cpu_sample():
        ...
        for pid in pids:
            try:
                ...
                # the man page counts from 1 so subtract 1 from field numbers
                p = proc_p / str(pid) / "stat"
                sample_time = time.monotonic()
                stats = p.read_text().split(" ")

                ppid = int(stats[3])
                children_by_ppid.setdefault(ppid, []).append(pid)

                proc_start_time[pid] = int(stats[21]) * clock_tick_duration
    ...
    uptime = float((proc_p / "uptime").read_text().split(" ")[0])
    ...
    def render_datum(x: Datum, *, indent: str = ""):
        ...
        table.append(
            [
                ...
                str(datetime.fromtimestamp(boot_time + x.start_time)),
                indent + x.comm,
